...
import { TransformWrapper, TransformComponent } from "react-zoom-pan-pinch"; // ZOOM
...
const BusinessDetailPage: React.FC = () => {
    ...
    const getWhatsappMessage = () => {
        ...
        if (categoryText === 'exchange' || categoryText === 'services') return `Hola, quisiera consultar el tipo de cambio en ${business.name}. Lo vi en Huaraz Explorer.`;
    ...
    // LÓGICA DEL TÍTULO DE LA CARTA/HABITACIONES
    const getMenuTitle = () => {
        ...
        if (categoryText === 'exchange' || categoryText === 'services') return { prefix: 'NUESTROS ', highlight: 'SERVICIOS', subtitle: 'Conoce nuestras opciones' };
        if (categoryText === 'health') return { prefix: 'NUESTRAS ', highlight: 'ESPECIALIDADES', subtitle: 'Conoce nuestros servicios médicos' };
        if (categoryText === 'hotel') return { prefix: 'NUESTRAS ', highlight: 'HABITACIONES', subtitle: 'Conoce nuestras opciones de descanso' };
    ...
    const getReservationData = () => {
        ...
        if (business.name.toLowerCase().includes('inka frut')) {
            ...
            return { tag: 'INSCRIPCIÓN', icon: 'fa-user-graduate', prefix: 'FORJA TU ', highlight: 'FUTURO', desc: 'Únete a la institución que forma a los líderes del mañana en la región.', btn: 'Solicitar Entrevista' };
        } else if (categoryText === 'exchange' || categoryText === 'services') {
            return { tag: 'ATENCIÓN RÁPIDA', icon: 'fa-money-bill-wave', prefix: 'CAMBIA CON ', highlight: 'SEGURIDAD', desc: 'Cotiza el mejor tipo de cambio de forma rápida y 100% confiable.', btn: 'Cotizar Cambio' };
        } else if (categoryText === 'health') {
            return { tag: 'ATENCIÓN MÉDICA', icon: 'fa-stethoscope', prefix: 'CUIDA TU ', highlight: 'SALUD', desc: 'Pon tu bienestar en manos de los mejores especialistas.', btn: 'Agendar Cita' };
        } else {
    ...
    return (
        <div className="bg-slate-50 dark:bg-slate-950 min-h-screen relative font-['Plus_Jakarta_Sans']">
            ...
            {/* HERO SECTION CON FONDO DINÁMICO OSCURECIDO */}
            <div 
                className="w-full h-[60vh] md:h-[70vh] relative overflow-hidden bg-slate-900"
                style={{ backgroundImage: `url('${business.photos[0]}')`, backgroundSize: 'cover', backgroundPosition: 'center' }}
            >
                <div className="absolute inset-0 bg-slate-950/60"></div> 
                <div className="absolute bottom-16 left-8 right-8 text-white max-w-5xl mx-auto z-10">
            ...
            <div className="max-w-5xl mx-auto px-6 -mt-12 relative z-10 space-y-16 pb-48">
                ...
                <div className="bg-white dark:bg-slate-900 rounded-[3.5rem] p-8 md:p-12 shadow-[0_40px_80px_rgba(0,0,0,0.08)] border border-slate-100 dark:border-slate-800 flex flex-col lg:flex-row gap-10 items-center justify-between">
                    ...
                    <div className="flex flex-wrap justify-center gap-4">
                        ...
                        {business.whatsapp && (
                            <a href={`https://wa.me/${business.whatsapp}?text=${whatsappMessage}`} target="_blank" rel="noreferrer" onClick={() => logClick(`Detalle - ${business.name}`)} className="w-16 h-16 md:w-20 md:h-20 bg-green-50 dark:bg-green-900/20 rounded-[2rem] flex items-center justify-center text-green-500 border border-green-100 dark:border-green-900/40 active:scale-95 transition-all shadow-sm">
                        ...
                        <button onClick={() => document.getElementById('review-section')?.scrollIntoView({ behavior: 'smooth' })} className="w-full bg-[#2A4D69] text-white px-10 md:px-14 py-6 md:py-8 rounded-[2rem] font-black uppercase text-[10px] md:text-xs tracking-[0.3em] shadow-2xl flex items-center justify-center gap-4 active:scale-95 transition-all">
                        ...
                        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 px-4">
                            ...
                            <div className="bg-slate-100 dark:bg-slate-800 px-8 py-3 rounded-full border border-slate-200 shadow-inner flex items-center gap-3">
                                ...
                                <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
                                    {categoryText === 'hotel' ? `${business.menuImages?.length} opciones` : `${business.menuImages?.length} páginas`}
                                </span>
                            </div>
                        </div>
                ...
                {/* HORARIOS Y TARJETA DE RESERVA DINÁMICA */}
                    ...
                    <div className="lg:col-span-3 bg-[#2A4D69] p-12 md:p-20 rounded-[4rem] shadow-[0_40px_100px_rgba(42,77,105,0.25)] text-white relative overflow-hidden group">
                        ...
                        <div className="relative z-10 space-y-8">
                            <div className="inline-flex items-center gap-4 bg-white/10 px-6 py-2.5 rounded-full border border-white/10">
                                <i className={`fas ${resData.icon} text-[#F58220]`}></i>
                            ...
                            <h3 className="text-4xl md:text-6xl font-black italic tracking-tighter uppercase leading-[0.9]">
                                {resData.prefix}
                                <span className="text-[#F58220]">{resData.highlight}</span>
                            </h3>
                ...
                <div className="space-y-12">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-6 md:gap-8 px-2">
                        {business.photos.map((photo, index) => (
                            <div key={index} className="aspect-square rounded-[3rem] overflow-hidden cursor-pointer shadow-xl hover:scale-[1.04] transition-all duration-700 border border-slate-100 dark:border-slate-800 relative group" onClick={() => openLightbox(index, 'gallery')}>
            ...
            {/* LIGHTBOX MODAL CON ZOOM INTEGRADO */}
            {isLightboxOpen && (
                <div className="fixed inset-0 z-[200] bg-slate-950/98 flex flex-col animate-fadeIn">
                    <div className="p-8 flex items-center justify-between text-white border-b border-white/5 bg-black/20 backdrop-blur-xl">
                        ...
                        <button onClick={closeLightbox} className="w-14 h-14 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-white/10 transition-all active:scale-90 z-[300]">
                            <i className="fas fa-times text-xl"></i>
                        </button>
                    </div>
                    
                    <div className="flex-grow relative flex items-center justify-center p-4 md:p-12 overflow-hidden">
                        <button onClick={prevImage} className="absolute left-4 md:left-12 top-1/2 -translate-y-1/2 text-white/30 hover:text-white w-20 h-20 rounded-full hover:bg-white/5 transition-all z-[300] flex items-center justify-center hidden md:flex">
                        ...
                        <TransformWrapper initialScale={1} minScale={1} maxScale={10}>
                            <TransformComponent wrapperStyle={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center" }}>
                                <img src={currentImages[currentImageIndex]} className="max-w-full max-h-[85vh] object-contain rounded-xl shadow-[0_0_150px_rgba(0,0,0,0.8)] border border-white/5 cursor-zoom-in" alt="Vista ampliada con zoom" />
                            </TransformComponent>
                        </TransformWrapper>

                        <button onClick={nextImage} className="absolute right-4 md:right-12 top-1/2 -translate-y-1/2 text-white/30 hover:text-white w-20 h-20 rounded-full hover:bg-white/5 transition-all z-[300] flex items-center justify-center hidden md:flex">
                            <i className="fas fa-chevron-right text-5xl"></i>
                        </button>
                    </div>

                    <div className="p-10 flex flex-col items-center gap-6 bg-black/40 backdrop-blur-md relative z-[300]">
