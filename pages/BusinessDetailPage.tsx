...
// 👇 ESPÍA BLINDADO Y SILENCIOSO (Sin console.log para que Vercel no llore) 👇
...
const BusinessDetailPage: React.FC = () => {
    ...
    const nextImage = (e: React.MouseEvent) => {
        e.stopPropagation();
        if (currentImages.length > 0) setCurrentImageIndex((prev) => (prev + 1) % currentImages.length);
    };

    const prevImage = (e: React.MouseEvent) => {
        e.stopPropagation();
        if (currentImages.length > 0) setCurrentImageIndex((prev) => (prev - 1 + currentImages.length) % currentImages.length);
    };

    // 👇 AQUÍ ARREGLÉ EL MAPA (Tenía 1{ en vez de ${ ) 👇
