...
{/* INNOVACIONES - PANEL DESLIZANTE */}
      <section className="animate-fadeIn">
        ...
        <div className="flex overflow-x-auto snap-x snap-mandatory pb-4 hide-scrollbar items-stretch">
          ...
          {/* SLIDE 1: EXPERIENCIA INMERSIVA VIDRYX (Ahora primero) */}
          <div className="w-full shrink-0 snap-center px-1 flex">
            {/* Reducción de largo: p-10 -> p-6 md:p-8, rounded-[3.5rem] -> rounded-[2.5rem] */}
            <div className="w-full bg-[#0A0A0A] rounded-[2.5rem] p-6 md:p-8 shadow-2xl text-white overflow-hidden relative border border-[#FFD700]/30 flex flex-col">
              ...
              <div className="relative z-10 flex flex-col h-full">
                {/* Reducción de espacios: mb-10 -> mb-6, h-20 -> h-16, text-4xl -> text-3xl */}
                <div className="flex items-center gap-4 mb-6">
                  <div className="w-16 h-16 bg-[#FFD700] rounded-[1.5rem] flex items-center justify-center text-black font-black text-2xl shadow-[0_0_20px_rgba(255,215,0,0.6)] shrink-0">VX</div>
                  <div>
                ...
                {/* Reducción de padding interior: p-8 -> p-6 */}
                <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-[2rem] p-6 mb-6 flex-grow flex flex-col justify-center">
                    <h4 className="text-2xl md:text-3xl font-black uppercase tracking-tighter mb-3 leading-tight">
                        {t('Explora en', 'Explore in')} <br/>
                        <span className="text-[#FFD700] italic">{t('Realidad Virtual', 'Virtual Reality')}</span>
                    </h4>
                    <p className="text-white/70 font-medium leading-relaxed mb-4 text-xs md:text-sm">
                    ...
                    <div className="flex flex-wrap gap-2">
                        <span className="bg-[#FFD700]/10 border border-[#FFD700]/30 text-[#FFD700] px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider flex items-center"><i className="fas fa-hotel mr-1.5"></i>{t('Hoteles', 'Hotels')}</span>
                        <span className="bg-[#FFD700]/10 border border-[#FFD700]/30 text-[#FFD700] px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider flex items-center"><i className="fas fa-utensils mr-1.5"></i>{t('Restaurantes', 'Restaurants')}</span>
                        <span className="bg-[#FFD700]/10 border border-[#FFD700]/30 text-[#FFD700] px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider flex items-center"><i className="fas fa-mountain mr-1.5"></i>{t('Paisajes', 'Landscapes')}</span>
                    </div>
                </div>
                
                {/* Reducción de botón: py-6 -> py-4 */}
                <button onClick={() => navigate('/inmersivo')} className="w-full bg-[#FFD700] text-black py-4 rounded-[1.5rem] font-black uppercase text-xs tracking-[0.3em] flex items-center justify-center gap-3 hover:bg-white transition-all shadow-[0_15px_30px_rgba(255,215,0,0.3)] mt-auto">
                    {t('Explorar en 360°', 'Explore in 360°')} <i className="fas fa-vr-cardboard text-lg"></i>
                </button>
              </div>
            </div>
          </div>

          {/* SLIDE 2: TOKEN $HUARAZ (Ajustado al mismo tamaño reducido) */}
          <div className="w-full shrink-0 snap-center px-1 flex">
            <div className="w-full bg-[#0A0A0A] rounded-[2.5rem] p-6 md:p-8 shadow-2xl text-white overflow-hidden relative border border-[#39FF14]/30 flex flex-col">
              ...
              <div className="relative z-10 flex flex-col h-full">
                ...
                <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-[2rem] p-6 mb-6 flex-grow">
                    ...
                    {/* Altura de gráfica reducida: h-32 -> h-20 */}
