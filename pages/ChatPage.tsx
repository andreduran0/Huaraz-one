...
const ChatPage: React.FC = () => {
  ...
  // Creamos un ID de sesión único para que la IA recuerde la conversación
  const [sessionId] = useState(() => `guia_24_7_${Date.now()}`);
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    try {
      // 1. Convertimos el historial al formato que entiende tu API
      const apiHistory = messages.map(m => ({
        role: m.sender === 'user' ? 'user' : 'assistant',
        content: m.text
      }));

      // 2. Llamamos a la API central (la misma que usa Arkáiko)
