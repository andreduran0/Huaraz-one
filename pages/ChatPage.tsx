            ...
            <div className={`flex flex-col gap-2 max-w-[85%] md:max-w-[75%]`}>
                ...
                <div className={`flex items-center gap-3 px-3 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
                    <span className={`text-[9px] font-black uppercase tracking-[0.2em] ${isUser ? 'text-slate-300' : 'text-[#FF5F1F]'}`}>
                        {isUser ? t('Viajero', 'Traveler') : 'Huaraz Explorer IA'}
                    </span>
                </div>
            </div>
        </div>
...
const ChatPage: React.FC = () => {
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { 
        ...
        text: response.text || t("No pude generar una respuesta.", "I couldn't generate a response."), 
        sender: 'ai'
      }]);
    } catch (error) {
      ...
      setMessages(prev => [...prev, { 
        id: 'error', 
        text: t(
            '🏔️ **Aviso:** No logramos conectar con el satélite en las montañas. Verifica tu conexión e inténtalo de nuevo.',
            '🏔️ **Notice:** We couldn\'t connect with the satellite in the mountains. Check your connection and try again.'
        ), 
        sender: 'ai' 
      }]);
    } finally {
  ...
  const suggestions = [
    { 
        text: t('¿Qué tours de un día recomiendas?', 'What day tours do you recommend?'), 
        icon: 'fa-route', 
        category: t('Aventura', 'Adventure') 
    },
    { 
        text: t('Comida típica que debo probar', 'Typical food I must try'), 
        icon: 'fa-plate-wheat', 
        category: t('Gastronomía', 'Gastronomy') 
    },
    { 
        text: t('¿Cómo llegar a la Laguna 69?', 'How to get to Laguna 69?'), 
        icon: 'fa-mountain-sun', 
        category: t('Guía', 'Guide') 
    },
    { 
        text: t('Próximas fiestas en Huaraz', 'Upcoming festivals in Huaraz'), 
          ...
          <div className="flex items-center gap-4">
              ...
              <div>
                  ...
                  <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-1">
                      {t('Guía Inteligente 24/7', '24/7 Smart Guide')}
                  </p>
              </div>
          </div>
          <button 
            ...
            title={t('Reiniciar conversación', 'Restart conversation')}
          >
            ...
            <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-12 animate-fadeIn py-12">
                ...
                <div className="space-y-4">
                    <h3 className="text-3xl font-black text-slate-900 tracking-tighter uppercase italic leading-none">
                        {t('¿Cómo puedo ', 'How can I ')}<span className="text-[#FF5F1F]">{t('ayudarte?', 'help you?')}</span>
                    </h3>
                    <p className="text-slate-400 font-bold text-[10px] tracking-widest uppercase">
                        {t('Expertos en la Cordillera Blanca', 'Experts in the Cordillera Blanca')}
                    </p>
                </div>
